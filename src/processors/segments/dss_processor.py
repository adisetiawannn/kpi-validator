# src/processors/segments/dss_processor.py
# Processor Revenue SEG_C (Segment CODE_C).
...
def _load_revenue_source(filepath, territory, year):
    ...
    mask = ((df["YEAR"]==year)&(df["INDICATOR"]=="REVENUE")&
            (df["SEGMENT"]=="CODE_C")&(df["TERRITORY"]==territory))
...
def _load_target(filepath, territory):
    ...
    mask = ((df["TERRITORY"]==territory)&(df["INDICATOR"]=="REVENUE")&(df["SEGMENT"]=="SEG_C"))
    row = df[mask]
...
def _load_historical(filepath, territory):
    return load_historical_revenue(filepath, territory, "SEG_C")
...
def calculate_dss(
    ...
    print(f"  [SEG_C] Loading revenue source...")
    df_rev = _load_revenue_source(revenue_file, territory, year)
    print(f"  [SEG_C] Loading target...")
    s_tgt  = _load_target(target_file, territory)
    print(f"  [SEG_C] Loading historical...")
    hist   = load_historical(historical_file, territory,"SEG_C","REVENUE") # update to load_historical(...)
    print(f"  [SEG_C] ✓ All data loaded")
    ...
    for idx, month in MONTH_NUMBER_MAP.items():
        ...
        if idx <= valid_idx:
            ...
            total   = sustain + corr + scaling + scal_otc
    ...
    return {"territory":territory,"segment":"SEG_C","year":year,
