# src/processors/segments/gs_processor.py
# Processor Revenue SEG_B (Segment CODE_B).
...
def _load_revenue_source(filepath, territory, year):
    ...
    mask = ((df["YEAR"]==year)&(df["INDICATOR"]=="REVENUE")&
            (df["SEGMENT"]=="CODE_B")&(df["TERRITORY"]==territory))
...
def _load_target(filepath, territory):
    ...
    mask = ((df["TERRITORY"]==territory)&(df["INDICATOR"]=="REVENUE")&(df["SEGMENT"]=="SEG_B"))
    row = df[mask]
...
def _load_historical(filepath, territory):
    return load_historical_revenue(filepath, territory, "SEG_B")
...
def calculate_gs(
    ...
    print(f"  [SEG_B] Loading revenue source...")
    df_rev = _load_revenue_source(revenue_file, territory, year)
    print(f"  [SEG_B] Loading target...")
    s_tgt  = _load_target(target_file, territory)
    print(f"  [SEG_B] Loading historical...")
    hist   = load_historical(historical_file, territory,"SEG_B","REVENUE") # update to load_historical(...)
    print(f"  [SEG_B] ✓ All data loaded")
    ...
    return {"territory":territory,"segment":"SEG_B","year":year,
